...
//////////
	...
	do
	{
		...
		delete autoPromotion; delete completionEvent;
	} while (status);
	if (Mode == silent)
		uiPtr->printSilent();
	uiPtr->outputFile();
...
void Station::addToCompletedMission()
{
	mission* m;
	MissionsINExecution.dequeue(m);
	MissionsCompleted.dequeue(m);
}
...
{
	rover* rV;
	bool Found = RoversINExecution.peek();
...
bool Station::isCompleted(int eD) //ADD
{
	...
	bool Found = MissionsINExecution.peek();
...
{
	WaitingMissions.removeItem(index);
}
//////////////////////////
///mustafa//////////
bool Station::assignMountainousMission(int eventDay)  //error
{
	//rami
	MissionsStat[0] ++;
 	//
	...
	//	MountainMission.removeItem(pMissionTemp->getID());
...
void Station::completeRover(int eD)
{
	rover* rV;
	bool Found = RoversINExecution.peek();
	while (Found && rV->getMissionOrCheckupEndDay() == eD)
	{
		...
		else
		{
			...
			if (rV->getneedMaintainance())
			{
				...
				if (eR)
					EmergencyRoversINCheckup.enqueue(eR);
				else if (mR)
					MountainRoversINCheckup.enqueue(mR);
				else
					PolarRoversINCheckup.enqueue(pR);
